...
abstract class TransactionsDao : BaseDao<EntityTransaction> {
    ...
    @Query(
        "SELECT " +
                ...
                "WHERE $TABLE_NAME.account_id=:accountId " +
                "ORDER BY $TABLE_NAME.timestamp DESC"
    )
    abstract suspend fun getTransactions(accountId: Long): List<TransactionInfo>
    ...
    abstract suspend fun getLastTransactions(): List<TransactionInfo>
